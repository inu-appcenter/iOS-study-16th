...
class Naver {
    ...
    // 나중에 함수 호출시 SearchNaver(input: keyword)대신 SearchNaver(keyword)로 간결하게 호출하려고 (_ input: String)으로 지정
    func SearchNaver(_ input: String) -> [Info]{
        // 각각의 글들을 저장할 Info형 빈 배열 InfoArray 생성
        // Info 구조체 구조 { title: String, link: String, preview: String }
        ...
        do {
            ...
            for index in 0..<min(titles.count, links.count, previews.count) {
                // InfoArray[[글1title, 글1link, 글1preview], [글2title, 글2link, 글2preview], ....]
                // 위와 같은 형식으로 저장하기 위해 Info형 구조체 인스턴스 생성 후 information 상수에 할당
